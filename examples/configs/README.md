...
| MMFlash | the DFlash knobs above (`training.loss_type` is the base weight of every token; `mmflash` makes it the prefix confidence `prod_{i<=k} ((1-s) p_i + s)` with `s` = `training.mmflash_smoothing` (`0.5`), i.e. the smoothed probability that verification reaches position k) plus `training.visual_alpha` (`1.0`; every token's weight is `base * (1 + alpha * g * (1 - p))` with `g` the visual grounding and `p` the draft's probability on the token; `0` is exactly `loss_type`) and the `data.visual_score_*` fields |
