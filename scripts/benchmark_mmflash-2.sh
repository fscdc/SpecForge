# DFlash baseline: our own drafter trained on llava-ov15-1M with prompt alignment,
# same benchmark list and settings as benchmark_mmflash.sh. Results resume into the
# existing dflash_baseline_llava_ov_1M_prompted_final_qwen35-4B_concurrency1_* files.
...
for idx in "${!GPU_IDS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${gpu_id} python3 -m sglang.launch_server \
        ...
        --speculative-draft-model-path /scratch/Projects/CFP-04/CFP04-CF-054/fengsicheng/specforge/draft_models/qwen3.5-4b-dflash-baseline-llava-ov15-1M-prompted-final \
...
python benchmarks/bench_mm.py \
    ...
    --top-k 20 \
    --max-tokens 4096 \
    --name dflash_baseline_llava_ov_1M_prompted_final_qwen35-4B_concurrency1_temp0_4096
...
python benchmarks/bench_mm.py \
    ...
    --top-k 20 \
    --max-tokens 4096 \
    --name dflash_baseline_llava_ov_1M_prompted_final_qwen35-4B_concurrency1_temp1_4096
...
#     --top-k 20 \
#     --max-tokens 8192 \
#     --name dflash_baseline_llava_ov_1M_prompted_final_qwen35-4B_concurrency1
