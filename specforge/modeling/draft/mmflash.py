"""MMFlash draft model: a byte-for-byte clone of the DFlash draft.

Forked from ``dflash.py`` so the MMFlash training strategy can evolve
independently. At the moment of the fork the two are identical apart from
names; everything below is free to diverge.

Deliberately NOT renamed, because they are formats shared with serving and the
rest of the family rather than algorithm identity:

* ``config.dflash_config`` -- the draft-config JSON key SGLang's DFLASH loader
  and ``scripts/gates/normalize_dflash_export.py`` read (``target_layer_ids``,
  ``mask_token_id``, ``block_size`` live there);
* ``DFlashKernels`` / ``DEFAULT_DFLASH_KERNELS`` -- the Qwen3 RMSNorm/MLP
  factory bundle in ``dflash_kernels.py``, also what the optional Liger path
  swaps in.

An exported MMFlash checkpoint is served by SGLang as ``DFlashDraftModel`` --
SGLang resolves the draft class by ``architectures[0]`` and only knows that
name -- which is exactly what the export normaliser rewrites it to.
"""
...
class Qwen3MMFlashAttention(nn.Module):
...
class Qwen3MMFlashDecoderLayer(GradientCheckpointingLayer):
    def __init__(
        self,
        config: Qwen3Config,
        layer_idx: int,
        ...
        self.self_attn = Qwen3MMFlashAttention(
            config=config,
            layer_idx=layer_idx,
            kernels=kernels,
        )
...
@register_draft
class MMFlashDraftModel(Qwen3PreTrainedModel):
    config_class = Qwen3Config
    _no_split_modules = ["Qwen3MMFlashDecoderLayer"]

    def __init__(
        self,
        config,
        mmflash_kernels: Optional[DFlashKernels] = None,
    ) -> None:
        super().__init__(config)
        self.config = config
        kernels = mmflash_kernels or DEFAULT_DFLASH_KERNELS
        self.layers = nn.ModuleList(
            [
                Qwen3MMFlashDecoderLayer(config, layer_idx, kernels)
    ...
    def _sample_draft_tokens(
        ...
        MMFlash predicts the whole suffix in one LM-head call. Draft families
